...
set global trandaction isolation level 隔离级别
...
如果隔离级别在read uncommitted

那么在未提交的数据修改会被另一半接收到导致错误的信息,而在数据被回滚后发生信息错误
...
第二范式（2NF）：在1NF的基础上，非码属性必须完全依赖于主码
...
码：如果在一张表中，一个属性或属性组，被其他所有属性所完全函数依赖，则称这个属性(属性组)为该表的码 例如：该表中码设置为（学号，课程名称）

  

函数依赖：A-->B,如果通过A属性(属性组)的值，可以确定唯一B属性的值。则称B依赖

于A 例如：学号-->姓名。 （学号，课程名称） --> 分数
