...
2. ip数据报分片和重组
	...
	-
	IP数据报分片的原因是因为在互联网上，不同的网络设备和链路都有各自的MTU，如果数据包的大小超过了某个设备或链路的MTU，那么这个数据包就无法通过这个设备或链路进行传输。因此，IP协议会将这个数据包分成若干个小的数据块，每个数据块的大小都不超过MTU，这样就可以保证数据能够在不同的设备和链路之间进行传输。
	-
