***网络编程三要素(IP地址,端口,协议)***
**TCP（传输控制协议）是一种面向连接的、可靠的、基于字节流的传输协议**
常用关键字
...
## TCP可靠传输如何实现
**TCP可靠传输如何实现**
	可靠传输概念:**可靠传输就是保证接收方收到的字节流和发送方发出的字节流是完全一样的**
	-
	1.序列号和确认应答机制(重要)
		接收方收到 TCP 报文段后就会返回一个确认应答消息
	2.校验和（Checksum）(了解即可)
...
**TCP的超时重传?**
	...
	（2）RTO设短了，重发的就快，会增加网络拥塞，导致更多的超时，更多的超时更多的重发。
	连接往返时间（RTT，Round Trip Time），指发送端从发送TCP包开始到接收它的响应所消耗的时间。
	**总结:超时重传时间 RTO 的值应该略大于报文往返 RTT 的值**
超时重传策略?
	1.如果超时重发的数据，再次超时的时候，又需要重传的时候，TCP 的策略是超时间隔加倍。
	也就是每当遇到一次超时重传的时候，都会将下一次超时时间间隔设为先前值的两倍。两次超时，就说明网络环境差，不宜频繁反复发送。
	2.TCP 还有另外一种 #快速重传机制 （Fast Retransmit），它不以时间为驱动，而是以数据驱动重传,   三次重复的ACK就会触发该机制
	结合SACK机制(Selective Acknowledgment)这种方式需要在 TCP 头部「选项」字段里加一个 SACK 的东西，它可以将缓存的地图发送给发送方，这样发送方就可以知道哪些数据收到了，哪些数据没收到，知道了这些信息，就可以只重传丢失的数据。保证了
	结合D-SACK(Duplicate-SACK)防止因为ACK信息丢失导致的信息重发问题和网络延迟导致的包重发问题,会在后续ACK告诉发送端下一个信息的目标范围这样「发送方」就知道了，数据没有丢，是「接收方」的 ACK 确认报文丢了
	Linux中通过net.ipv4.tcp_sack和net.ipv4.tcp_dsack参数打开开关(linux2.4后默认开启)
**TCP的滑动窗口:**
	**为什么需要滑动窗口:**
	![[TCP协议_image_8.png|220]]![[TCP协议_image_9.png|370]]
	信息之间传送的缓冲大小,本质是批量传输(一次最多能接收到多少信息而不需要回传)
	滑动窗口数据重传问题:
	1.数据包已经抵达，ACK（确认应答包）被丢了?
		这种情况下，如果未使用滑动窗口机制，发送的数据包没有收到确认应答包，那么数据都会被重发；如果使用了滑动窗口机制，即使确认应答包丢失，也不会导致数据包重发，因为可以通过后续的ACK进行确认(Duplicate-SACK)
	2.数据包直接丢了
		这种情况指的是发送端发送的部分数据没有达到接收端。那么，如果接收端收到的数据包，不是本该要接受的数据包，就是给发送端返回消息，告诉发送端自己应该接收的数据包。
		如果发送端连续收到3次这样的数据包，就认为该数据包成功发送到接收端，这时就重发该数据包。这种机制叫做 #快速重传机制
		接收端收到重发数据包以后，查看这次是自己应该接受的数据包2001-3000，并返回确认应答包，告诉发送包，下一个该接受8001的数据包了因为3001-8000接收端其实之前就收到了，被放到了接收端操作系统内核的接受缓冲区
	![[TCP协议_image_10.png|400]]
	**滑动窗口大小**
...
**TCP流量控制**
	...
	原理:
	在通信过程中，接收方根据自己接收缓存的大小，动态调整发送方的发送窗口大小，即TCP报文段首部中的“窗口”字段rwnd,来限制发送方向网络注入报文的速率。同时，发送方根据其对当前网络拥塞程序的估计确定一个拥塞窗口cwnd，最终A发送的窗口的实际大小是min(rwnd,cwnd)值。不断调整
流量控制的窗口关闭问题?
	**造成原因:** 接收方向发送方通告窗口大小时，是通过 `ACK` 报文来通告的。
	那么，当发生窗口关闭时，接收方处理完数据后，会向发送方通告一个窗口非 0 的 ACK 报文，如果这个通告窗口的 ACK 报文在网络中丢失了这会导致发送方一直等待接收方的非 0 窗口通知，接收方也一直等待发送方的数据，如不采取措施，这种相互等待的过程，会造成了死锁的现象。
	**解决方案:** TCP 为每个连接设有一个持续定时器，只要 TCP 连接一方收到对方的零窗口通知，就启动持续计时器。
	如果持续计时器超时，就会发送窗口探测 ( Window probe ) 报文，而对方在确认这个探测报文时，给出自己现在的接收窗口大小。
	如果接收窗口仍然为 0，那么收到这个报文的一方就会重新启动持续计时器；
	如果接收窗口不是 0，那么死锁的局面就可以被打破了。
	窗口探测的次数一般为 3 次，每次大约 30-60 秒（不同的实现可能会不一样）。如果 3 次过后接收窗口还是 0 的话，有的 TCP 实现就会发 RST 报文来中断连接。
流量控制糊涂窗口症状
	**原因**:当发送端应用进程产生数据很慢、或接收端应用进程处理接收缓冲区数据很慢，或二者兼而有之；就会使应用进程间传送的报文段很小，特别是有效载荷很小；
	**解决方案**:-   
	1.让接收方不通告小窗口给发送方
	...
	收到之前发送数据的 `ack` 回包
**TCP拥堵控制**
	**为什么需要拥堵控制:** 在网络出现拥堵时，如果继续发送大量数据包，可能会导致数据包时延、丢失等，这时 TCP 就会重传数据，但是一重传就会导致网络的负担更重，于是会导致更大的延迟以及更多的丢包，这个情况就会进入恶性循环被不断地放大
	**如何确认发生拥堵:** 只要「发送方」没有在规定时间内接收到 ACK 应答报文，也就是发生了超时重传，就会认为网络出现了用拥塞。
	**拥堵控制算法:**
	1.慢启动
		从小的量(MSS大小)开始发送,当发送方每收到一个 ACK，拥塞窗口 cwnd 的大小就会加 1   指数增长
		有一个叫慢启动门限 `ssthresh` （slow start threshold）状态变量达到启动门限后切换成避免算法
	2.避免算法:
		每个确认增加 1/8，8 个 ACK 确认 cwnd 一共增加 1，于是这一次能够发送 9 个 `MSS` 大小的数据，变成了线性增长
	3.超时重传算法
