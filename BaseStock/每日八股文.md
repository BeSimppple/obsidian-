...
59.数组,链表,树的插入和搜索性能排序?
	...
	**树**:
	优点:查询速度和层数相关.层数越少查询越快,同时增删速度也很快
	所以数据库结构是b+树
62.Collection 和 Collections的区别。
	Collection是集合的上级接口,
	Collections是一个帮助类提供一系列静态方法实现对各种集合的搜索、排序、线程安全化等操作
...
68.java中有几种类型的流？JDK为每种类型的流提供了一些抽象类以供继承，请说出他们分别是哪些类？
	字节流InputStream OutputStream   常用:FileInputStream和BufferedInputStream
	字符流 InputStreamReader OutputStreamWriter  常用:InputStreamReader和BufferedReader。
69.字节流与字符流的区别
	字节流是最基本的,采用ASCII编码可以处理任意数据,但是不能直接处理Unicode字符
	字符流采用Unicode编码是要虚拟机进行字符集的转化它通常处理文本数据，
70.什么是java序列化，如何实现java序列化？或者请解释Serializable接口的作用。
	序列化是将代码转换成字节流.通过实现Serializable接口标记该类可被序列化,然后通过writeObject和readObject方法来手动控制序列化和反序列化的过程
71.描述一下JVM加载class文件的原理机制?
	通過ClassLoader類和其子类實現的加載
	1.  **加载（Loading）**：在加载阶段，JVM虚拟机会通过类加载器（ClassLoader）查找并加载Class文件。类加载器会根据类的全限定名（Fully Qualified Name）查找Class文件，如果找不到则会抛出ClassNotFoundException异常。
	2.  **验证（Verification）**：在验证阶段，JVM虚拟机会对Class文件进行验证，确保其符合Java语言规范和JVM规范。验证阶段包括文件格式验证、元数据验证、字节码验证和符号引用验证。
	3.  **准备（Preparation）**：在准备阶段，JVM虚拟机会为Class文件中的静态变量分配内存并设置默认值。例如，对于一个int类型的静态变量，JVM虚拟机会为其分配4个字节的内存并将其值设置为0。
	4.  **解析（Resolution）**：在解析阶段，JVM虚拟机会将类中的符号引用转换为直接引用。例如，将类中对其他类的引用转换为对其他类的实际引用。
	5.  **初始化（Initialization）**：在初始化阶段，JVM虚拟机会执行类的初始化代码。在Java中，静态变量的初始化代码和静态代码块中的代码都会在初始化阶段执行。
72.heap和stack有什么区别。
	主要区别栈内存用来存储局部变量和方法调用。 而堆内存用来存储Java中的对象
73.GC是什么? 为什么要有GC?
	Garbage Collection 内存处理是编程人员容易出现问题的地方，忘记或者错误的内存回收会导致程序或系统的不稳定甚至崩溃，Java提供的GC功能可以自动监测对象是否超过作用域从而达到自动回收内存的目的
...
65.两个对象值相同(x.equals(y) == true)，但却可有不同的hash code，这句话对不对?
	如果是在set或map中则不对,如果是在list中则没任何问题因为没有hashcode方法
